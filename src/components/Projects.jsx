...
import { Github, ExternalLink, Sprout, Plane, Film, Utensils, Leaf, Zap } from "lucide-react";
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {projects.map((project, index) => {
              ...
              return (
                <Card
                  key={index}
                  className="bg-card border-border hover:border-primary/40 hover:shadow-xl transition-all duration-300 group cursor-pointer"
                  onClick={() => setSelectedProject(index)}
                >
                  <CardHeader>
                    <div className={`w-12 h-12 rounded-lg bg-gradient-to-br ${project.color} p-2.5 mb-4 group-hover:scale-105 transition-transform shadow-md`}>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="text-2xl flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${projects[selectedProject].color} p-2`}>
                    {(() => {
